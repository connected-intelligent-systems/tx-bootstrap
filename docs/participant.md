...
| `PARTICIPANT_PORTAL_DATA_PROXY_ALLOWED_PRIVATE_HOSTS` | preview allowlist                                    | Comma-separated exact EDR endpoint hostnames that may resolve to private addresses through the application data proxy.                                   |
| `PARTICIPANT_PORTAL_DATA_PROXY_TIMEOUT_MS`            | `300000`                                             | Maximum duration in milliseconds for one proxied application data request.                                                                               |
| `PARTICIPANT_PORTAL_DATA_PROXY_MAX_REQUEST_BYTES`     | `52428800`                                           | Maximum streamed request-body size accepted by the application data proxy.                                                                               |
...
Asset metadata may contain one OpenAPI 3.x description uploaded as JSON or YAML. The portal stores the normalized OpenAPI document directly as metadata, not as an executable endpoint description. `servers`, operation-level server overrides, API security declarations, callbacks, webhooks, external references, and external documentation values are removed. After a consumer receives access, the portal binds the document to the participant HTTP-pull proxy for that transfer. Swagger requests use the administrator's existing browser session and the browser never receives the EDR endpoint or authorization token.

### Application data proxy

Machine clients with the `data:proxy` scope can call negotiated HTTP data through
`/api/data/{transferProcessId}`. Additional path segments and query parameters
are appended to the EDR endpoint, and GET, HEAD, POST, PUT, PATCH, and DELETE are
forwarded with streamed bodies and responses. The participant backend resolves
and refreshes the EDR internally, so endpoint and authorization credentials are
never returned to the application. The provider asset must enable the matching
HTTP proxy capabilities (`proxyPath`, `proxyQueryParams`, `proxyMethod`, and
`proxyBody`) for the requested operation.

The `edr:data-address:read` scope remains available for compatibility. New
applications should use `data:proxy` so EDR endpoints, access tokens, and token
refresh stay inside the participant backend.
